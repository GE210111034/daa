...
int dfs(Graph* graph, int start, int end, int visited[]) {
    ...
    for (int i = 0; i < graph->vertices; i++) {
        if (graph->adjacencyMatrix[start][i] == 1) {
            if (!visited[i]) {
...
int main() {
    ...
    Graph graph;
    initializeGraph(&graph, vertices, 0);
    ...
    if (isPathExists(&graph, start, end)) {
        printf("Path exists between the given vertices.\n");
    } else {
        printf("Path does not exist between the given vertices.\n");
    }
