...
bool isBipartite(Graph* graph) {
    ...
    int front = 0, rear = 0;
    int start = 0; // Start vertex for BFS
    ...
    while (front != rear) {
        ...
        if (graph->adjacencyMatrix[current][current] == 1) {
            return false;
        }
        ...
        for (int i = 0; i < graph->vertices; i++) {
            if (graph->adjacencyMatrix[current][i] == 1) {
                // If the neighbor is not colored, assign a different color and enqueue
                if (color[i] == 0) {
                    color[i] = (color[current] == 1) ? 2 : 1;
                    queue[rear++] = i;
                }
