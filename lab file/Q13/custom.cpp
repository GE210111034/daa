...
char* randArrayChar(int size, int dpt) {
    char* arr = new char[size];
    char start = 'A', end;

    if (!dpt) {
        end = start + size - 1;
        // Generate a range of unique characters
        char* uniqueArr = new char[end - start + 1];
        for (int i = 0; i < end - start + 1; i++) {
            uniqueArr[i] = start + i;
        }
        // Shuffle the unique characters
        srand(time(nullptr));
        std::random_shuffle(uniqueArr, uniqueArr + end - start + 1);
        ...
        // Generate random characters with the possibility of duplicates
        srand(time(nullptr));
        for (int i = 0; i < size; i++) {
            arr[i] = start + rand() % (end - start + 1);
        }
